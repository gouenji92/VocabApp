...
def delete_comment(comment_id: str, user_id: str) -> bool:
    ...
    - Xóa kèm likes của comment
    - Xóa tất cả replies thuộc comment và likes của các replies đó
    """
    comments = _load(COMMENTS_FILE)
    for i, comment in enumerate(comments):
        if comment.get('id') == comment_id and comment.get('user_id') == user_id:
            # Xóa comment khỏi danh sách
            removed_comment = comments.pop(i)
            _save(COMMENTS_FILE, comments)

            # Xóa likes của comment này
            try:
                likes = _load(COMMENT_LIKES_FILE)
                filtered = [l for l in likes if l.get('comment_id') != comment_id]
                if len(filtered) != len(likes):
                    _save(COMMENT_LIKES_FILE, filtered)
            except Exception:
                pass

            # Xóa tất cả replies và likes của replies thuộc comment này
            try:
                replies = _load(COMMENT_REPLIES_FILE)
                reply_ids = [r.get('id') for r in replies if r.get('comment_id') == comment_id]
                # Ghi lại replies không thuộc comment
                remaining_replies = [r for r in replies if r.get('comment_id') != comment_id]
                _save(COMMENT_REPLIES_FILE, remaining_replies)

                if reply_ids:
                    rlikes = _load(REPLY_LIKES_FILE)
                    rlikes_filtered = [l for l in rlikes if l.get('reply_id') not in reply_ids]
                    if len(rlikes_filtered) != len(rlikes):
                        _save(REPLY_LIKES_FILE, rlikes_filtered)
            except Exception:
                pass
            return True
    return False
...
def delete_reply(reply_id: str, user_id: str) -> bool:
    ...
    - Xóa kèm tất cả likes của reply
    """
    replies = _load(COMMENT_REPLIES_FILE)
    for i, reply in enumerate(replies):
        if reply.get('id') == reply_id and reply.get('user_id') == user_id:
            ...
            try:
                rlikes = _load(REPLY_LIKES_FILE)
                rlikes_filtered = [l for l in rlikes if l.get('reply_id') != reply_id]
                if len(rlikes_filtered) != len(rlikes):
                    _save(REPLY_LIKES_FILE, rlikes_filtered)
            except Exception:
                pass
